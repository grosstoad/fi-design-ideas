...
## 10. Addendum — owner design direction from Paper (July 2026, verified 2026-07-04)

The owner supplied two Paper frames and written direction after the v1 spec; spec v2 encoded that direction from description alone (frames were not fetchable in that session). This session fetched both frames directly — `get_screenshot`, `get_jsx`, and `get_computed_styles` against the live Paper canvas — and spec v3 corrects several details that the written description alone got wrong. **FILL-IN #27 is now resolved.**

**Direction received, and what the frames actually show:**
1. **Dual headline stat** — lender detail shows max property price and loan amount at the *same* hierarchy: price left / loan right per the owner's explicit instruction. **Correction from v2:** the frames do not show a coloured "forest banner" holding both stats — that was v2's inference, not something in Paper. The actual built pattern (Paper `01KSYP7T3MFEQHHED41F3PQB58/3-0`, card "03 Capacity pair" in the "Selected card hierarchy variations" row) is two plain `--field`-bg tiles side by side, each with an 11.5px muted label and a 24px/800 value — no colour block, no white-on-forest text. Node `19MF-0` ("M3 Statement card", the frame the owner specifically called out as "the best representation of how max property price is displayed") shows property price alone as a single large row (24px/800), not paired with loan — the pairing comes from the sibling "Capacity pair" card, and the owner's brief explicitly asks to combine the two ("it needs the equivalent loan amount"). Spec §7.6 item 2 and §10.5 rewritten to the tile pattern.
2. **Equal-prominence rates (legal requirement)** — interest rate and comparison rate always the same size and prominence. **Confirmed exactly as described** — every card variant inspected in the "Core flow row" and "Selected lender detail variations" row uses the identical Tailwind class for Rate and Comparison, with zero exceptions. No correction needed here; v2's spec language stands. Spec §7.6 item 3, §10.5, §12a.
3. **Funds to complete** — keep the compact CTA + "View breakdown" pattern (confirmed: node `191L-0` "03 Selected plus funds action" shows exactly this compact row, and `19BH-0` "D Funds receipt" shows the expanded breakdown reached by tapping it — this is the click-to-expand interaction the owner asked for). **Correction from v2:** the `4WM-0` reference ("Funds chart card") is not two mirrored breakdown lists — it's one categorised breakdown (stacked bar + legend: property price / stamp duty / transfer+legal / lender fees, each with a % share) followed by a three-figure verdict strip (Funds required / Available funds / Remaining cash + %). Spec §8 rewritten to reflect this exact structure, keeping Group 2 ("where it comes from") as a deliberate addition beyond the reference since this product (unlike `4WM-0`'s single savings input) can have multiple funding sources.

**What this resolves from the earlier audit:**
- The "two competing headline numbers" concern (§7 desktop item 1, review v1) is now settled at the *card* level: the card leads with a dual stat, and loan amount leaves the tiles. The page-level hero recommendation (9.5) still stands and is compatible.
- The comparison-rate demotion flagged in the design export (comparison as a sub-label under the interest tile) is now formally banned and directly confirmed in the reference designs, retiring 9.13's UI-side risk — 9.13's other half (never client-adjust a published comparison rate) still applies.
- The funds block regains the "sources" dimension the old design.md had (available funds), strengthening the 9.2 usable-deposit recommendation: Group 2's total is exactly the "available funds" figure in the verdict strip.
- **Banner geometry question resolved:** there is no banner. The dual stat is two tiles at 24px/800 each (not 32px on a forest block as v2 speculated) — this matches the actual scale relationship in `19MF-0` and "03 Capacity pair".

**New questions raised by the direction (FILL-INs #25–#26 remain; #27 resolved):**
...
3. **Consistency sweep** — one was found and fixed immediately: the `/landing-a` proof card displayed a lender interest rate with no comparison rate. Under A14 every existing and future surface needs the same sweep (results prototypes, share cards, the assessment page's lender table).

## 11. Addendum 2 — full artboard sweep (2026-07-06)

The owner asked whether every screen on Paper artboard `18WZ-0` had been reviewed. It hadn't — the 2026-07-04 pass covered the cards relevant to the three explicit directions. This session swept all five rows (Core flow 01–05, Header copy variations, Selected card hierarchy variations, Selected lender detail variations A–E, Detail editing destinations, Stronger selected-card variations M1–M5) plus the artboard's board note. The sweep **reversed two adopted decisions and replaced the token set**; spec v3 now encodes all of it.

**What the sweep changed:**

| Was (v2 / earlier v3) | Now (verified) | Why |
| --- | --- | --- |
| A6: sort cut for v1 | **Sort reinstated** — "Sort" affordance on the default screen + "Sort lenders" sheet with 8 options and the caption "Sorting changes the order, not your calculation" (spec §7.3a) | Card "05 Sort sheet" is part of the core flow row, not an exploration; the board note's "Separate controls" principle names sorting explicitly |
| A3: live re-rank on 300ms debounce while editing | **Explicit "Save and recalculate"** — edits happen in dedicated Update details sheets; recalc fires on save-and-return with rows shimmering in place | Cards "04 Update details sheet" + the "Detail editing destinations" row all end in a Save-and-recalculate CTA; the financial-inputs frame carries the literal note "After save, Results returns with stable recalculating lender rows" |
| Secondary CTA "Edit financials" + in-card "Edit loan details" disclosure | Single **"Update details"** entry → chooser (Property details / Loan details / Financial inputs) → per-area sheets; no in-card edit affordance | Every frame's CTA stack is "Connect with a broker" + "Update details"; no card variant contains an edit disclosure |
| Tokens: Hanken Grotesk, cream `#F4F3ED` page, forest `#15362A` CTA/accent (from Results.html) | **Helvetica Neue, white page, teal `#167D7F` links, mint `#85C7BE` controls/progress, black `#111111` CTA**; 8px radii; flat (no card shadows); weights 400/700 only | Extracted from the frames' computed styles — the Results.html token set matches nothing on the artboard. Resolves FILL-IN #16 |
| Progress bar removed on results (A10) | **Progress bar stays** — 7 segments, all filled mint | Present in every frame on the artboard |
| Hero copy "Your highest estimated property price — with {lender}…" | **"You may be able to afford a property up to {$X.XXM}."** + H1 "Max property price by lender" (25px) + figure (38px) | Verbatim from "01 Default results"; board-note principle "No redundant copy" |
| — (unspecced) | **Non-top-selected compare footer** (§7.8a): leader + selected mini-rows + neutral caption, "without a scolding paragraph" | Cards "E Non-top selected" / "M4 Compare footer" |
| — (unspecced) | Property sheet fields: State, Purpose, First home buyer, Savings, Property price in mind (optional), Capitalise purchase costs toggle. Loan sheet: term / repayment type (+ conditional IO term) / product type / rate type (+ conditional fixed term) | "Detail editing destinations" row, read directly |

**Resolved by the sweep:** #1 (source of truth = Paper), #5c (edit scope global by construction), #6-part (state editable via Property sheet), #16 (palette/type). **Recommendation 9.11 (cut sort) is formally withdrawn** — the owner's design includes it; the review's cost argument stands only against *recalculating* sorts, which the Paper caption explicitly rules out anyway.

**New questions (added to spec §14):** #28 desktop frames (artboard is mobile-only; desktop values extrapolated), #29 "Policy fit" sort option needs a backing metric or gets cut, #30 "Property price in mind" engine behaviour. One caution: the frames brand the header "fundora" and show a "Menu" button — both treated as placeholders (fundiq wordmark and Save & exit stand, per A10); flag if either is actually intended.
